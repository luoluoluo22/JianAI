    ...
    for (const action of actions) {
      ...
      const createdAsset = addAsset(currentProjectId, result.mediaType === 'video'
        ? {
            ...
            path: result.path,
            url: result.url,
            managedPaths: [result.path, result.htmlPath, result.thumbnailPath].filter((value): value is string => Boolean(value)),
        ...
        : {
            ...
            path: result.path,
            url: result.url,
            managedPaths: [result.path, result.htmlPath].filter((value): value is string => Boolean(value)),
            prompt: action.name.trim() || 'HTML 素材',
            resolution: `${result.width}x${result.height}`,
            duration: action.duration,
          })
